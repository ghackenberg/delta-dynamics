import { TILE_SIZE, OFFSET, BOUNDARY, GRID_SIZE, SKIN_TONES, OUTFIT_COLORS } from '../constants/gameConfig'
import type { Human, Animal, BuildingInstance } from '../types/game'
import { gridToWorld } from '../utils/gameUtils'

export const findSafeLandPosition = (sWater: Float32Array): [number, number] => {
  for (let attempt = 0; attempt < 100; attempt++) {
    const x = Math.floor(Math.random() * GRID_SIZE), z = Math.floor(Math.random() * GRID_SIZE)
    if (sWater[z * GRID_SIZE + x] <= 0.05) return [x * TILE_SIZE - OFFSET, z * TILE_SIZE - OFFSET]
  }
  return [0, 0]
}
...
export const updateHuman = (human: Human, isNight: boolean, buildings: BuildingInstance[], sWater: Float32Array): Human => {
  ...
  if (newTarget) {
    ...
    if (dist > 0.1) {
      ...
      if (gx >= 0 && gx < GRID_SIZE && gz >= 0 && gz < GRID_SIZE && sWater[gz * GRID_SIZE + gx] <= 0.05) { newPos[0] = nextX; newPos[1] = nextZ }
    }
  }
...
export const updateAnimal = (animal: Animal, sWater: Float32Array): Animal => {
  ...
  if (newState === 'IDLE') {
    ...
    if (newWait > (5 + Math.random() * 10) * 20) {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (gx >= 0 && gx < GRID_SIZE && gz >= 0 && gz < GRID_SIZE && sWater[gz * GRID_SIZE + gx] <= 0.05) { newTarget = [tx, tz]; break }
      }
    }
